...
void GetData(byte _ns) {
  ...
  Serial.println("Lux:");
  Serial.println( map(Rsensor,0,160, 100,0));
  Lecturas[2] = Lecturas[2] + Rsensor;
...
void SetData(byte _ns){
    boolean _send = false;
    float _tiempo = millis() - lastConnectionTime ; 
    Last[0] = Lecturas[0]/LCounterT;
    
    if(Lecturas[1]>0){
         Last[2] = Lecturas[1]/LCounterH; 
    }
    if(Lecturas[2]>0){
         Last[4] = Lecturas[2]/LCounterL; 
    }
    if( _tiempo > 60000) {  
         ...
         if(_send){
            httpRequest( UrlEncodedata(_nsonda) ); 
         }else{
            Serial.println( UrlEncodedata(_nsonda) );
         }
...
String UrlEncodedata(byte _ns){
   String _exit = "&_s=" + String(_ns);
  _exit = _exit + "&L=" + Last[4] ; //map(Last[4],0,160, 100,0) ;
  _exit = _exit + "&H=" + Last[2] + "&T=" + Last[0] + + "&_vt=" + Last[1] + "&_vh=" + Last[3];;
  _exit = _exit  + "&type=SHT11&_le=" + LCounterT;
  if(_BmpSonda==1){
      _exit = _exit + "&px=" + bmp.readPressure() ;
      _exit = _exit + "&hx=" + bmp.readAltitude() ;
     // _exit = _exit + "&tx=" + bmp.readTemperature() ;
  } 
   if(InitSonda){
        _exit = _exit + "&_Go=1";
        InitSonda= false;
